...
const thisPtr = '__PTR__';  // expose the pointer of wasm object
...
export function instantiateSync(module, imports) {
...
/** Instantiates an AssemblyScript module from a buffer (<4kB) using the specified imports. */
export function instantiateBufferSync(buffer, imports) {
  return instantiate(new WebAssembly.Module(buffer), imports);
}

/** Instantiates an AssemblyScript module from a buffer using the specified imports. */
export async function instantiateBuffer(buffer, imports) {
  return postInstantiate(
    preInstantiate(imports || (imports = {})),
    (await WebAssembly.instantiate(buffer, imports)).instance  // to fit wasm file > 4KB
  )
}

/** Instantiates an AssemblyScript module from a response using the specified imports. */
export async function instantiateStreaming(response, imports) {
  const buffer = await response.then(res => res.arrayBuffer());  // to fit mine-type not 'application/wasm'
  return postInstantiate(
    preInstantiate(imports || (imports = {})),
